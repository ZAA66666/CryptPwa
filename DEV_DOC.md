...
- **数据隐私**：数据备份与同步（WebDAV 备份/恢复，自动连通性检测；本地导出/导入 JSON）
...
**安卓原生能力**：沉浸式状态栏（默认开）、全局返回键（弹窗→设置→面板→**双击退出**）、Toolbar 三点菜单（设置/关于）、状态栏安全区留白、**小窗拖放接收文字/图片**、按钮圆角点击高亮、overscroll 禁止整页滑动、**固定签名 p12**（不再换签名）。
