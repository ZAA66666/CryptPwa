...
完整的逐功能图文教程见 **[使用教程.md](使用教程.md)**。

- 所有工具共用「输入 → 执行 → 结果」三步式操作
- 涉及密钥/口令的功能（哈希 HMAC、加解密）执行后可选择**保存到密码本**，下次一键填入（算法、模式、密钥长度、IV 自动恢复）
- 设置页支持：主题/强调色、字体大小、语言（中/英/自导入）、数据加密、WebDAV 备份同步、外部调用与分享、实验性功能
...
- **流程**：检出代码 → 安装 Node/Java → 用 Capacitor 生成安卓工程 → Gradle 打包 debug APK → 创建 tag → 发布到 Releases
